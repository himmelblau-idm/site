...
This parameter is **REQUIRED** for successful authentication. If it is
not specified, no users will be permitted to authenticate.
...
is recommeneded. By name mapping is the default.
...
PIN. If not set, the default prompt is:
...
password. If not set, the default prompt is:
...
permitted when MFA is unavailable. Disabled by default.
...
specified in sudo_groups. Only has an affect if sudo_groups is set.
...
requiring a password. By default, this option is disabled.
...
Specifies the hostname for Microsoft authentication. The default value
is **login.microsoftonline.com**.
...
The timeout for connections to the authentication server. Default is 2
seconds.
...
If this option is not set, the default path */etc/himmelblau/user-map*
is used.
...
If this option is set to **false** (the default), Himmelblau will
continue to cache password verifiers only for SFA-only users, and
MFA-enabled users will **not** be able to authenticate when offline. The
**aad-tool**(1) command **aad-tool offline-breakglass** will also have
no effect.
